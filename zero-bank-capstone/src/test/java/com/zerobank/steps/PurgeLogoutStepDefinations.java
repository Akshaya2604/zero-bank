...
import com.zerobank.pages.PdfDownloadingPage;
import com.zerobank.pages.PurgeLogoutPage;
...
public class PurgeLogoutStepDefinations {

	WebDriver driver = Base.driver;
	ExtentTest test = Hooks.test;
	PdfDownloadingPage downloadingPage;
	PurgeLogoutPage logoutPage;
	...
	public void the_user_is_logged_into_the_zero_bank_platform_with_and(String username, String password) {
		try {
			downloadingPage = new PdfDownloadingPage(driver, test);
	...
	public void the_user_clicks_on_the_logout_button() {
		try {
			logoutPage = new PurgeLogoutPage(driver, test);
			logoutPage.logout();
		} catch (Exception e) {
			Assert.fail("Error occured: " + e.getMessage());
		}
